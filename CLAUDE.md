...
Each closed sketch shape (rect, circle, closed polygon) carries `operation: 'add' | 'subtract'` (default `'add'`). The worker composes solids in 3D: every additive shape is `sketchOnPlane(plane).extrude(depth)` and fused into a running `added` solid; every subtractive shape is extruded the same way and cut from `added` sequentially. This matches replicad's documented pattern (`house.cut(window).fuse(door)`) and — crucially — handles **multiple disjoint pairs** correctly. An earlier 2D-boolean pipeline (`Drawing.fuse` to union additives, then `Drawing.cut` to subtract) silently dropped the second pair when given two sets of concentric circles, because compound `Drawing` objects don't always survive `cut` cleanly; doing the booleans on 3D solids avoids that. Lines and open polylines are construction-only and aren't passed to the worker. Subtractive shapes render in the Sketcher with red dashed outlines and pink fill so they're distinguishable; the properties panel exposes a `Union / Difference` toggle (`OperationToggle`) for each closed shape.
